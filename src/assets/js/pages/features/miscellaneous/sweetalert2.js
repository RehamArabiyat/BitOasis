...
var KTSweetAlert2Demo = function () {
	var _init = function () {
		...
		$('#kt_sweetalert_demo_9').click(function (e) {
			Swal.fire({
				title: 'هل أنت متأكد من إصدار الوثيقة؟',
				icon: 'warning',
				showCancelButton: true,
				confirmButtonText: 'نعم, إصدر الوثيقة الآن',
				cancelButtonText: 'لا, إلغاء الطلب!',
				reverseButtons: true
			}).then(function (result) {
				if (result.value) {
					Swal.fire(
						'تم إصدار الوثيقة بنجاح!',
						'وبإمكانك طباعة الوثيقة من خلال الرابط التالي',
						'success'
					).then(function (result) {
						if (result.value) {
						  window.open('policy-details.html',"_self");
						}
					})
					...
					Swal.fire(
						'إلغاء الطلب',
						'تم إلغاء الاصدار بناء على طلبك :)',
						'error'
					)
				}
			});
		...
		$('#kt_sweetalert_demo_12').click(function (e) {
			Swal.fire({
				title: 'هل أنت متأكد من إصدار (650) وثيقة بإجمالي (969,450 ر.س)؟',
				icon: 'warning',
				showCancelButton: true,
				confirmButtonText: 'نعم, إصدر الوثائق الآن',
				cancelButtonText: 'لا, إلغاء الطلب!',
				reverseButtons: true
			}).then(function (result) {
				if (result.value) {
					Swal.fire(
						'تم إرسالل طلبك بنجاح!',
						'وسيتم إشعاركم فور إكتمال الطلب',
						'success'
					).then(function (result) {
						if (result.value) {
						  window.open('multiproposalstatus.html',"_self");
						}
					})
					// result.dismiss can be 'cancel', 'overlay',
					// 'close', and 'timer'
		...
		$('#kt_sweetalert_demo_13').click(function (e) {
			Swal.fire({
				title: 'هل أنت متأكد من تجديد الوثيقة؟',
				icon: 'warning',
				showCancelButton: true,
				confirmButtonText: 'نعم, جدد الوثيقة الآن',
				cancelButtonText: 'لا, إلغاء الطلب!',
				reverseButtons: true
			}).then(function (result) {
				if (result.value) {
